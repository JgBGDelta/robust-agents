...
- **Tamaño de muestra**: `sample_size_labels()` añade `(n=<tamaño>)` bajo cada etiqueta de configuración cuando el diagrama agrupa por `agent_id`/`configuration_id`. Para tasas de resolución, `n` es el **total de instancias** del grupo (denominador de SAN.6.3.3), no el número de filas con `resolved` no nulo.
...
- **Tasas de resolución**: `resolution_metrics.resolution_rate_pct()` (SAN.6.3.3) calcula `resolved == true` / total de filas del grupo. `plot_rate_bar()` + `configure_rate_axis()` fijan el eje Y en 0–100 % y escriben la etiqueta porcentual incluso cuando la tasa es 0 % real.
...
- **Datos faltantes vs. cero real**: si un grupo no tiene filas (`len(subset) == 0`), se anota `"sin datos"`. Si hay filas pero ninguna resuelta (`resolved == true` en ninguna), la tasa es **0 % real** sobre el total de instancias. Los valores `null`/`false` de `resolved` permanecen en el denominador; no se usa `dropna` sobre `resolved` para calcular la tasa principal. El conteo `evaluable` es métrica secundaria y no sustituye al denominador.
...
- **`resolution_metrics.py`** - definición centralizada de la tasa de resolución (SAN.6.3.3 / EB.8.4): `resolution_rate_pct()`, `resolution_rate_stats()`, `resolution_rate_delta_pp()`. Todas las figuras con tasas de resolución deben importarla; no recalcular con `mean()` sobre `resolved.dropna()`.
...
├── resolution_metrics.py                # Tasa de resolución centralizada (SAN.6.3.3)
